...
from weibo_spider.WeiboCommentScrapy import WeiboCommentScrapy
...
class Spider:
    def __init__(self, config):
        """Weibo类初始化"""
        self.read_comment=config["read_comment"]
        self.comment_page=config["comment_page"]
            ...
            if since_date <= now:
                ...
                for page in tqdm(range(1, page_num + 1), desc='Progress'):
                    ...
                    if self.read_comment:
                        for weibo in weibos:
                            try:
                                weibo.comment = WeiboCommentScrapy(weibo.id,self.comment_page).run()
                            except Exception:
                                print("评论爬取失败")
                    logger.info(
